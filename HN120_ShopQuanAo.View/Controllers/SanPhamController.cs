...
namespace HN120_ShopQuanAo.View.Controllers
{
    public class SanPhamController : Controller
    {
        ...
        public async Task<IActionResult> CreateSanPham(SanPham bk, IFormFile imageFile)
        {
            ...
            var urlBook = $"https://localhost:7197/api/SanPham/add-TL?Tensp={bk.TenSP}&MaThuongHieu={bk.MaThuongHieu}&MaTheLoai={bk.MaTheLoai}&MoTa={bk.Mota}&TongSoLuong={bk.TongSoLuong}&TrangThai={bk.TrangThai}&UrlAvatar={bk.UrlAvatar}";
            var httpClient = new HttpClient();
            var content = new StringContent(JsonConvert.SerializeObject(bk), Encoding.UTF8, "application/json");
            var respon = await httpClient.PostAsync(urlBook, content);
            if (respon.IsSuccessStatusCode)
            {
                return RedirectToAction("AllSanPhamManager", "SanPham");
            }
