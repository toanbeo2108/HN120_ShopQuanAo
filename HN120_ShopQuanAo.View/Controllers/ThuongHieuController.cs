...
namespace HN120_ShopQuanAo.View.Controllers
{
    public class ThuongHieuController : Controller
    {
        ...
        public async Task<IActionResult> CreateThuongHieu(ThuongHieu bk)
        {
            ...
            var urlBook = $"https://localhost:7197/api/ThuongHieu/add-TL?Tenth={bk.TenThuongHieu}&MoTa={bk.MoTa}&TrangThai={bk.TrangThai}";
            var httpClient = new HttpClient();
            var content = new StringContent(JsonConvert.SerializeObject(bk), Encoding.UTF8, "application/json");
            var respon = await httpClient.PostAsync(urlBook, content);
            if (respon.IsSuccessStatusCode)
            {
                return RedirectToAction("AllThuongHieuManager", "ThuongHieu");
            }
