...
namespace HN120_ShopQuanAo.View.Controllers
{
    public class TheLoaiController : Controller
    {
        ...
        public async Task<IActionResult> CreateTheLoai(TheLoai bk)
        {
            ...
            var urlBook = $"https://localhost:7197/api/TheLoai/add-TL?Tentl={bk.TenTheLoai}&MoTa={bk.MoTa}&TrangThai={bk.TrangThai}";
            var httpClient = new HttpClient();
            var content = new StringContent(JsonConvert.SerializeObject(bk), Encoding.UTF8, "application/json");
            var respon = await httpClient.PostAsync(urlBook, content);
            if (respon.IsSuccessStatusCode)
            {
                return RedirectToAction("AllTheLoaiManager", "TheLoai");
            }
